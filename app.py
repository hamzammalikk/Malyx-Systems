...
from road_aggregation import aggregate_by_road_name, bulk_set_congestion
...
with tab_priority:
    ...
    if has_road_name:
        ...
        st.markdown('<div class="section-tag">Bulk-fill congestion for a whole road at once</div>', unsafe_allow_html=True)
        st.caption(
            "Instead of entering congestion segment by segment, set one value for every segment "
            "sharing a road name. Download the updated CSV and re-upload it in the sidebar to apply."
        )
        ...
        with bc2:
            new_congestion = st.slider("New congestion (1-10)", 1, 10, 5)
        if st.button("Apply to all segments of this road"):
            updated_edges = bulk_set_congestion(edges_df, road_choice, new_congestion)
            n_updated = (updated_edges["road_name"] == road_choice).sum()
            st.success(f"Updated congestion to {new_congestion} for all {n_updated} segment(s) of \"{road_choice}\".")
            st.download_button(
