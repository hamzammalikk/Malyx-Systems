...
def cached_simulation(edges_df, node_demands, decay_rate, mu, generations):
...
result = cached_simulation(edges_df, node_demands, decay_rate, mu, generations)
...
with tab_baseline:
    ...
    st.caption(
        "An honest check: this compares the full 30-generation adaptation simulation against a "
        "single-step static score (the same demand/impedance ratio, computed once, with no "
        "iteration). If the two rankings mostly agree, the simulation isn't adding much for "
        "ranking purposes -- worth knowing before claiming the bio-inspired approach is better."
    )
    ...
    if corr is not None and corr > 0.9:
        st.warning(
            ...
            f"On the current model, each corridor evolves independently during simulation (no edge "
            f"ever influences another), so the 30-generation adaptation is mathematically guaranteed "
            f"to preserve the same order as this one-line static formula. For *ranking* purposes, "
            f"the simulation isn't currently adding information a much simpler calculation wouldn't "
            f"give you. It still adds value elsewhere -- the simulation-verification step in the "
            f"Cost-Benefit tab uses the same nonlinear response curve to model diminishing returns on "
            f"an intervention, which a linear formula wouldn't capture. But the core priority ranking "
            f"claim should be described as grounded in established theory, not as empirically "
            f"outperforming a simple method -- because right now, it doesn't."
        )
